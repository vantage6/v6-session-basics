...
# Read full dataframe and split into 2 random subsets
...
def test_two_df_sum_function(mock_client: MockUserClient):
    """Test the two_df_sum function"""
    # Get organizations
    ...
    assert len(results) == len(org_ids)  # One organization
    assert (
        results[0]["sum"]
        == TEST_DATAFRAME_1[column_to_sum].sum() + TEST_DATAFRAME_2[column_to_sum].sum()
    )
